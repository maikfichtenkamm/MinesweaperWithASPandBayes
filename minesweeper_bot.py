''' 
This python script is mostly based on the code from gamescomputersplay published under the MIT license.
The bot was written to play different Minesweeper versions. 
I have shortened the code so it is only vor the Beginner Board. 
You can find the published code at https://github.com/gamescomputersplay/minesweeper-solver/blob/main/minesweeper_bot.py.
The code was published by Github / YOututbe account gamescomputersplay with the help of Justin Golden,

Bot that plays minesweeper "from pixels", including finding the gamebs
field on the screen, recognizing the numbers, clicking cells etc.
Uses minesweeper solver bayes_solver and asp_clingo_solver.py
depending on the command line input of a user, the choosen solver is used. 
For bayes_solver: type -bs
For asp_clingo_solver:
    for my own soltuion: -asp-own
    for dissertation: -asp-dis
...
import asp_clingo_solver as asp
import bayes_solver as bs

"""
helper variables for defining staes of the board
"""
CELL_MINE = -1
CELL_COVERED = -2
CELL_FALSE_MINE = -3
...
"""
helper functions for defining the status of the game
"""
STATUS_ALIVE = 0
STATUS_DEAD = 1
STATUS_WON = 2
...
class MinesweeperBotSettings():
    ...
    def __init__(self, field_color, samples_files,
                 cell_padding=1, click_pause=0.3, sample_sensitivity=3000):
...
SETTINGS_MINESWEEPER_CLASSIC = MinesweeperBotSettings(
    ...
    samples_files={
        ...
        "./samples/msx-7.png": 7,
        "./samples/msx-8.png": 8,
        "./samples/msx-mine.png": CELL_MINE,
        "./samples/msx-flag.png": CELL_MINE,
        "./samples/msx-covered.png": CELL_COVERED,
        "./samples/msx-explosion.png": CELL_EXPLODED_MINE,
    }
)
...
class MinesweeperBot:
    ...
    def find_game(self, image=None):
        ...
        try:
            if sys.argv[1] == '-asp-own':
                self.solver = asp.ASPSolver(mode="own")
            elif sys.argv[1] == '-asp-dis':
                self.solver = asp.ASPSolver(mode="dis")
            else:
                self.solver = bs.BayesSolver()
        except IndexError:
            print(
                "Enter the correct argument for a specific solver: options: -bs, -asp-own, -asp-dis")
            return False
    ...
    def is_dead(self, field):
        ...
        for i in range(self.game_shape[0]):
            for j in range(self.game_shape[1]):
                if field[i, j] == CELL_EXPLODED_MINE:
                    return True
        return False
...
def use_bot(games_to_play=100, settings=SETTINGS_MINESWEEPER_CLASSIC,
    ...
    for game in range(games_to_play):
        ...
        while True:
            ...
            if result == STATUS_DEAD:
                print("I died.", end=" ")
                break
            if result == STATUS_WON:
                print("I won.", end=" ")
                wins += 1
                break
        winrates.append(wins / (game+1))
